...
import affinity.com.srisabaries.RetrofitClient;
import affinity.com.srisabaries.adapters.FlipperAdapter;
import affinity.com.srisabaries.network.ApiClient;
import affinity.com.srisabaries.network.IApiClient;
import affinity.com.srisabaries.network.ProductDetails;
...
import affinity.com.srisabaries.ui.fragments.ChallengesFragment;
import affinity.com.srisabaries.ui.fragments.ConnectAppsAndDevicesFragment;
import affinity.com.srisabaries.ui.fragments.FavouriteFragment;
import affinity.com.srisabaries.ui.fragments.FsStoreFragment;
import affinity.com.srisabaries.ui.fragments.HomeFragment;
import affinity.com.srisabaries.ui.fragments.MyAccountFragment;
import affinity.com.srisabaries.ui.fragments.MyCartFragmentNew;
import affinity.com.srisabaries.ui.fragments.MyWalletFragment;
...
import affinity.com.srisabaries.utils.CustomTypefaceSpanNavigationDrawer;
...
public class HomeActivity extends BaseActivity implements NavigationView.OnNavigationItemSelectedListener, SharedPreferences.OnSharedPreferenceChangeListener {
	...
	affinity.com.srisabaries.customviews.CustomTypefaceTextView tvToolbarCartCount;
	@BindView(R.id.fab)
	ImageView fab;
	@BindView(R.id.tv_toolbar_favourite_count)
	affinity.com.srisabaries.customviews.CustomTypefaceTextView tvToolbarCount;
	...
	private ArrayList<ProductDetails> productsArrayList;
	private RecyclerView product_all_cat;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapterViewFlipping = (AdapterViewFlipper)findViewById(R.id.adapterFLiper);
	...
	private void getBanners() {
		IApiClient api = RetrofitClient.getApiService();

		/*String user_id = mAppSharedPreference.getString(PreferenceKeys.KEY_USER_ID, getApplicationContext().getString(R.string.txt_skip_user_id));
		String keys = mAppSharedPreference.getString(PreferenceKeys.KEY_SERVICE_KEY, ServiceConstants.SERVICE_KEY);*/

		String user = mAppSharedPreference.getString(PreferenceKeys.KEY_USER_ID, "");
		String method = "getAllSliders";
		String key = mAppSharedPreference.getString(PreferenceKeys.KEY_SERVICE_KEY, ServiceConstants.SERVICE_KEY);
		String page = "1";
		Call<Banners> call = api.getBanners(mAppSharedPreference.getString(PreferenceKeys.KEY_SERVICE_KEY, "")/*user,method,key,"",page*/);
		call.enqueue(new Callback<Banners>() {
			@Override
			public void onResponse(Call<Banners> call, Response<Banners> response) {
				Banners res =response.body();
				if (response.isSuccessful()) {

					dismissProgressDialog();
					ArrayList<BannerList> banners = response.body().getBanners().get(0).getBannerLists();
	...
	protected void initViews() {
		android.support.v4.app.FragmentTransaction fragmentTransaction;
		//ivHomeBottom.setSelected(true); // testing command for bottom bar view hiding
		...
		mFragmentStack.push(homeFragment); // testing command for bottom bar view hiding
		...
		/*tvWorkout.setOnClickListener(this);  // testing command for bottom bar view hiding
		...
		/*for (int i = 0, size = arcLayout.getChildCount(); i < size; i++) { // testing command for bottom bar view hiding
		...
		*/fab.setOnClickListener(this);
		...
		if (intent.getExtras() != null) {
			if (intent.getExtras().getString(AppConstants.TAG_FROM_WHERE, "") != null && intent.getExtras().getString(AppConstants.TAG_FROM_WHERE, "").equals(AppConstants.FROM_NOTIFICATION_ACTIVITY)) {
				/*showChallengesFragment();*/ // testing command for bottom bar view hiding
	...
	public boolean onNavigationItemSelected(MenuItem item) {
		...
		/*hideMenu();
		rlParentLayoutArcMenu.setBackgroundColor(0x00000000); // testing command for bottom bar view hiding
		*/fab.setImageResource(R.drawable.menu_list_bulleted_24);
		...
		switch (item.getItemId()) {
			case R.id.nav_home:
				...
				setVisibilityBottomLayout(true);
				/*ivWalletBottom.setSelected(false); // testing command for bottom bar view hiding
			...
			case R.id.nav_settings:
				...
				adapterViewFlipping.setVisibility(View.GONE);
			...
			/*case R.id.nav_connect:
				if (mAppSharedPreference.getBoolean(PreferenceKeys.KEY_LOGGED_IN, false)) {
					if (mAppSharedPreference.getString(PreferenceKeys.KEY_IS_EMAIL_VERIFIED, "0").equalsIgnoreCase("1")) {
						*//*setVisibilityBottomLayout(false);*//* 		// testing command for bottom bar view hiding
...
//                }
	 ...
	 */
	public void startFsStoreFragment() { // testing command for bottom bar view hiding
		ivTrophyBottom.setSelected(false);
		...
		rlNavigationToolbarFavourite.setVisibility(View.GONE);
		//if (mAppSharedPreference.getBoolean(PreferenceKeys.KEY_LOGGED_IN, false)) {
		//    if (mAppSharedPreference.getString(PreferenceKeys.KEY_IS_EMAIL_VERIFIED, "0").equalsIgnoreCase("1")) { // testing command for bottom bar view hiding
...
//        }
	...
	public void startHomeFragment() {
		...
		ivToolbarShare.setVisibility(View.GONE);
//            } else {
//                AppDialog.showVerifyEmailAlertDialog(this); // testing command for bottom bar view hiding
//            }
//        } else {
//            AppDialog.showLoginAlertDialog(HomeActivity.this);
//        }
	...
	public void onBackPressed() {
		...
		/*else {
			if (!(mCurrentFragment instanceof HomeFragment)) {
				...
				customToolbar.setVisibility(View.GONE);
			*//*	ivWalletBottom.setSelected(false);
				ivTrophyBottom.setSelected(false);// testing command for bottom bar view hiding
				ivHomeBottom.setSelected(true);
				ivFsStoreBottom.setSelected(false);*//*
				AppUtilMethods.hideKeyBoard(this);
			} else {
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			...
			case R.id.myacc:
				...
				break;
			case R.id.fab:
                intent = new Intent(HomeActivity.this, RecommendedActivity.class);
                startActivity(intent);
				break;

			// testing command for bottom bar view hiding
	...
	/*private void showChallengesFragment() {
		...
		if (mAppSharedPreference.getBoolean(PreferenceKeys.KEY_LOGGED_IN, false)) {
			if (mAppSharedPreference.getString(PreferenceKeys.KEY_IS_EMAIL_VERIFIED, "0").equalsIgnoreCase("1")) {
				...
				unCheckNavigationSelectedItem();
				rlNavigationToolbarFavourite.setVisibility(View.GONE);
				fragmentTransaction = getSupportFragmentManager().beginTransaction();  // testing command for bottom bar view hiding
	...
	/*private void onFabClick(View v) {
		if (fab.isSelected()) {
			...
			showMenu();
			rlParentLayoutArcMenu.setBackgroundColor(getResources().getColor(R.color.c_black_overlay)); // testing command for bottom bar view hiding
	...
	/*private void showMenu() {
		...
		for (int i = 0, len = arcLayout.getChildCount(); i < len; i++) {
			animList.add(createShowItemAnimator(arcLayout.getChildAt(i)));   // testing command for bottom bar view hiding
		}
	...
	/*private void hideMenu() {
		...
		for (int i = arcLayout.getChildCount() - 1; i >= 0; i--) {
			animList.add(createHideItemAnimator(arcLayout.getChildAt(i)));   // testing command for bottom bar view hiding
		}
	 ...
	 * @return
	 */
	/*private Animator createShowItemAnimator(View item) { // testing command for bottom bar view hiding
	 ...
	 * @return
	 */
	/*private Animator createHideItemAnimator(final View item) { // testing command for bottom bar view hiding
		float dx = fab.getX() - item.getX();
		float dy = fab.getY() - item.getY();
		Animator anim = ObjectAnimator.ofPropertyValuesHolder(
				item,
