...
public class HomeAllCatFragment  extends BaseFragment {
    ...
    private ResGetProductsCategories ResGetProductsCategories;
    private List<ResProductsCategoryData> resTrendingProductsData = new ArrayList<>();

    @BindView(R.id.rcvTrending)
    RecyclerView rcvTrending;

    HomeAllCatFragment.TrendingPageAdapter adapter;
    ...
    public void getTrendingCouponsAndProductsForHomeTabs() {
        ...
        reqTrendingCoupons.setMethod(MethodFactory.GET_PRODUCT_CATEGORIES.getMethod());
...
///for trending products
        final ReqProductsCategory reqTrendingProducts = new ReqProductsCategory();
        ...
        reqTrendingProducts.setMethod(MethodFactory.GET_PRODUCT_CATEGORIES.getMethod());
       ...
       // reqTrendingProducts.setFilterType(null);
        ...
        Call<ResGetProductsCategories> ResGetProductsCategoriesCall = client.getProductsCategory(  mAppSharedPreference.getString(PreferenceKeys.KEY_SERVICE_KEY, "")/*reqTrendingProducts*/);
        ResGetProductsCategoriesCall.enqueue(new Callback<ResGetProductsCategories>() {
            ...
            public void onResponse(Call<ResGetProductsCategories> call, Response<ResGetProductsCategories> response) {
                ResGetProductsCategories = response.body();
                if (ResGetProductsCategories != null) {
                    if (ResGetProductsCategories.getSuccess() == ServiceConstants.SUCCESS) {
                        resTrendingProductsData.addAll(ResGetProductsCategories.getProductCatData());
                        //setTrendingProductImages();
                        setAdapterData();
                        //setProductsClickListeners();
                    } else {
                        //ToastUtils.showShortToast(mActivity, ResGetProductsCategories.getErrstr());
...
//    }
    ...
    private void openProductDetailScreen(int pos) {
        Intent intent;
       /* if (resTrendingProductsData.get(pos).getProCatID().equals(ServiceConstants.FS_STORE_PRODUCT)) {*/
            intent = new Intent(mActivity, ProductsListActivity.class);
            intent.putExtra("productId", resTrendingProductsData.get(pos).getCategory_id());
       /* } else {
            intent = new Intent(mActivity, ProductDetailActivity.class);
            intent.putExtra("productId", resTrendingProductsData.get(pos).getProCatID());
        }*/
    ...
    class TrendingPageAdapter extends RecyclerView.Adapter<HomeAllCatFragment.TrendingPageAdapter.TrendingViewHolder>{

        List<ResProductsCategoryData> mlistResTrendingProductsData;

        @Override
        public HomeAllCatFragment.TrendingPageAdapter.TrendingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.trending_page_inflate,parent,false);
            return new HomeAllCatFragment.TrendingPageAdapter.TrendingViewHolder(itemView);
        }

        public TrendingPageAdapter(List<ResProductsCategoryData> mlistResTrendingProductsData){
            this.mlistResTrendingProductsData= mlistResTrendingProductsData;
        }

        public void setData(List<ResProductsCategoryData> mlistResTrendingProductsData){
            this.mlistResTrendingProductsData= mlistResTrendingProductsData;
        }

        @Override
        public void onBindViewHolder(final HomeAllCatFragment.TrendingPageAdapter.TrendingViewHolder holder, int position) {
            ResProductsCategoryData data = mlistResTrendingProductsData.get(position);
            Glide.with(mActivity)
                    .load(data.getOriginal_image())
                    .error(R.drawable.no_image_available)
                    .listener(new RequestListener<String, GlideDrawable>() {
                        @Override
                        public boolean onException(Exception e, String model, Target<GlideDrawable> target, boolean isFirstResource) {
